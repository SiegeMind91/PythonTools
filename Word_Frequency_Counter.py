...
def count_me():
	...
	file_obj = open('count.txt', 'r') #Open the local file, count.txt
	word_content = file_obj.read().lower() # Turn all the words in our document into lowercase.

	reg_exp = re.findall(r'\b[a-z]{3,15}\b', word_content) #Create a regEx expression that you want to match against the file
	...
	freq_list = freq.keys() #Get all of the words that were given

	for word in freq_list:
		print(word, freq[word]) #Print the word and the frequency
