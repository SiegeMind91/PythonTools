...
def seq_matrix_multiply(A, B):
    ...
    if num_cols_A != num_rows_B:
        raise ArithmeticError(f'Invalid dimensions; Cannot multiply {num_rows_A}x{num_cols_A}*{num_rows_B}x{num_cols_B}')
    # compute a return matrix product C = A * B
    C = [[0] * num_cols_B for i in range(num_rows_A)] # Here we're creating the matrix C that will hold the values from the multiplication
